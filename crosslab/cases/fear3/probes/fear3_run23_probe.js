/**
 * FEAR 3 Steam P2P Diagnostic Probe - Run 23 (Full Teardown Caller Evidence)
 * 
 ...
 * 360-second safety timeout to cover full match lifecycle until teardown burst.
 * 
 * Usage:
 *   frida -n "F.E.A.R. 3.exe" -l fear3_run23_probe.js
 */

console.log("[CrossLab Probe] Initializing Run 23 entry-only diagnostic probe in F.E.A.R. 3...");

const steamApi = Process.findModuleByName("steam_api.dll") || Process.findModuleByName("steam_api64.dll");

if (!steamApi) {
    console.log("[CrossLab Probe] RUN23 PREFLIGHT ABORT: required steam_api.dll/steam_api64.dll is not loaded");
    throw new Error("RUN23 PREFLIGHT ABORT: required steam_api.dll/steam_api64.dll is not loaded");
}

const run23ReviewedManifest = {
    fear3: {
...
const validateReviewedImage = function(module, expected) {
    ...
    console.log(`[CrossLab Probe] Run23 manifest OK ${module.name} sha256=${sha256} pe_timestamp=0x${identity.peTimestamp.toString(16)} size_of_image=0x${identity.sizeOfImage.toString(16)}`);
};

const run23MainModule = Process.mainModule;
const run23SteamClient = Process.findModuleByName("steamclient.dll");
try {
    if (!run23MainModule) {
        throw new Error("required main module is not available");
    }
    validateReviewedImage(run23MainModule, run23ReviewedManifest.fear3);
    validateReviewedImage(run23SteamClient, run23ReviewedManifest.steamclient);
    console.log("[CrossLab Probe] Run23 preflight manifest OK; stalker-free entry-only mode active");
} catch (error) {
    console.log(`[CrossLab Probe] RUN23 PREFLIGHT ABORT: ${error}`);
    throw error;
}

const run23InstalledListeners = [];
let run23Finished = false;
let run23Captured = false;
let timeoutGuard = null;

const cleanupRun23 = function(reason) {
    if (run23Finished) {
        return;
    }
    run23Finished = true;
    ...
    while (run23InstalledListeners.length > 0) {
        const listener = run23InstalledListeners.pop();
        try {
            listener.detach();
        } catch (_) {}
    }
    console.log(`[CrossLab Probe] Run23 detached all hooks (reason: ${reason})`);
};

const requiredAttach = function(address, label, callbacks) {
    if (!address || address.isNull()) {
        cleanupRun23(`cannot resolve ${label}`);
        throw new Error(`RUN23 PREFLIGHT ABORT: cannot resolve ${label}`);
    }
    try {
        const listener = Interceptor.attach(address, callbacks);
        run23InstalledListeners.push(listener);
        console.log(`  [+] Attached ${label} at ${address}`);
        return listener;
    } catch (error) {
        cleanupRun23(`required hook failed: ${label}`);
        throw new Error(`RUN23 PREFLIGHT ABORT: required hook failed: ${label}: ${error}`);
    }
};
...
if (!legacyNetworkingVtable || Process.pointerSize !== 4) {
    throw new Error("RUN23 PREFLIGHT ABORT: reviewed x86 legacy ISteamNetworking vtable unavailable");
}
...
if (!closeOwner || closeOwner.name.toLowerCase() !== run23ReviewedManifest.steamclient.moduleName.toLowerCase()) {
    throw new Error(`RUN23 PREFLIGHT ABORT: CloseP2PChannelWithUser target ${closeChannelTarget} owner=${closeOwner ? closeOwner.name : "none"}`);
}
const closeTargetRva = closeChannelTarget.sub(closeOwner.base).toUInt32();
if (closeTargetRva !== run23ReviewedManifest.steamclient.closeChannelRva) {
    throw new Error(`RUN23 PREFLIGHT ABORT: CloseP2PChannelWithUser target ${closeOwner.name}+0x${closeTargetRva.toString(16)} expected +0x${run23ReviewedManifest.steamclient.closeChannelRva.toString(16)}`);
}
if (!bytesMatch(closeChannelTarget, run23ReviewedManifest.steamclient.closeChannelEntryBytes)) {
    throw new Error(`RUN23 PREFLIGHT ABORT: CloseP2PChannelWithUser entry byte signature mismatch at ${closeOwner.name}+0x${closeTargetRva.toString(16)}`);
}
console.log(`[CrossLab Probe] Run23 live-owner preflight OK target=${closeChannelTarget} authority=${closeOwner.name}+0x${closeTargetRva.toString(16)} entry_bytes=OK`);

// Arm the 360s Safety Timeout Guard BEFORE hook installation
timeoutGuard = setTimeout(function() {
    cleanupRun23("360s safety timeout reached");
}, 360000);

// Primary Run 23 Hook: CloseP2PChannelWithUser (Slot 5)
requiredAttach(closeChannelTarget, "Run23 CloseP2PChannelWithUser target", {
    onEnter: function(args) {
        if (run23Finished || run23Captured) {
            return;
        }
        ...
        const evidence = {
            event: "Run23CloseP2PChannelEvidence",
        ...
        console.log("[Client Probe] Control channel 4101 close detected. Detaching Run 23 probe.");

        setImmediate(function() {
            cleanupRun23("control channel 4101 captured");
        });
    }
});

console.log("[CrossLab Probe] Run 23 probe attached successfully. Waiting for channel 4101 event...");