...
PROBE_RUN23 = (
    ...
    / "fear3"
    / "probes"
    / "fear3_run23_probe.js"
)


def test_run23_is_entry_only_and_stalker_free() -> None:
    source = PROBE_RUN23.read_text(encoding="utf-8")
...
def test_run23_preflight_is_pinned_to_reviewed_images_and_entry_bytes() -> None:
    source = PROBE_RUN23.read_text(encoding="utf-8")
    ...
    assert "bytesMatch(closeChannelTarget, run23ReviewedManifest.steamclient.closeChannelEntryBytes)" in source
    assert "RUN23 PREFLIGHT ABORT" in source


def test_run23_hooks_target_vtable_slot_5_with_fail_closed_rollback() -> None:
    source = PROBE_RUN23.read_text(encoding="utf-8")
    ...
    assert "cleanupRun23" in source
    assert "run23InstalledListeners.pop()" in source


def test_run23_captures_immediate_return_address_and_bounded_backtrace_with_synchronous_latch() -> None:
    source = PROBE_RUN23.read_text(encoding="utf-8")

    assert "if (run23Finished || run23Captured) {\n            return;\n        }\n        const retAddr = this.returnAddress;\n        const channel = args[2].toInt32();" in source
    assert "if (channel !== 4101) {\n            return;\n        }" in source
    assert "if (run23Captured) {\n            return;\n        }\n        run23Captured = true;" in source
    ...
    assert "Run23CloseP2PChannelEvidence" in source


def test_run23_timer_initialized_before_hook_installation() -> None:
    source = PROBE_RUN23.read_text(encoding="utf-8")
    ...
    assert 'cleanupRun23("control channel 4101 captured")' in source
    assert "360000" in source


def test_run23_probe_hashes_match_reviewed_candidate() -> None:
    canonical = PROBE_RUN23.read_text(encoding="utf-8").encode("utf-8")
    git_blob = hashlib.sha1(
        b"blob " + str(len(canonical)).encode("ascii") + b"\0" + canonical
    ).hexdigest()
    sha256 = hashlib.sha256(canonical).hexdigest()

    assert git_blob == "7fcc5fe4f5be61d157238d9cf375c8d166947dfc"
    assert sha256 == "31bd90247a4280cefbca2c944f7edd75acb63bf3ec1a1c03ddfe7c467d5023ad"